...
| `src/telemetry/`                    | Error reports to the core's own Sentry project for the app's daemon only: pure `reports.ts` (what each failure becomes, or nothing), `scrub.ts`, `stack.ts`, `envelope.ts`; `reporter.ts` sends (consent, dedup, caps), `daemon.ts` the process handlers, `subscribe.ts` the engine events. |
