...
| `src/diffusion/`                    | Image generation on stable-diffusion.cpp, a module of its own (not a `LocalRuntime`). Pure: `args.ts` (argv, `img_gen` body), `progress.ts` + `tracker.ts` (reading `sd-server`'s output), `parse.ts` + `validate.ts` (request bodies), `recipe.ts`, `workflow.ts`, `errors.ts`. Disk: `png.ts` (dependency-free codec), `gallery.ts`, `install.ts`, `containment.ts`. Wire types live in `src/contracts/diffusion.ts`. |
