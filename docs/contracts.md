...
| --- | --- | --- | --- |
| Error codes `{code,message,details?}` | `src/contracts/errors.ts`, `src/runtime/llamacpp/errors.ts` (classified per provider) | `src-tauri/plugins/tauri-plugin-llamacpp-upstream/src/error.rs`; TurboQuant's `tauri-plugin-llamacpp/src/error.rs` differs in `wrong number of tensors` → `MODEL_ARCH_NOT_SUPPORTED` (image-generation line `ec1fd3ea7`) | `test/fixtures/app/errors/*.json` (upstream); the TurboQuant rule in `errors.test.ts` and `runtime.test.ts` |
...
| Image generation wire types (camelCase; absent vs `null` as the plugin's serde produced), the 23 `DiffusionErrorCode`s incl. `ENGINE_UPDATE_REQUIRED` (409) and `INVALID_OUTPUT` (500) (statuses informational: 400/404/409/429/507/500), `files.llmVision`, events `diffusion:{state,progress,job,error}` | `src/contracts/{diffusion,errors,events}.ts`, `src/diffusion/{parse,errors}.ts`, `src/server/http.ts` | image-generation line `web-app/src/services/diffusion/types.ts`, plugin `tauri-plugin-atomic-diffusion/src/{state,error,events}.rs` at `ec1fd3ea7` | `contracts/errors.test.ts` (code list), `diffusion/parse.test.ts`, `server/http.test.ts` |
| `sd-server` argv (`--llm_vision` after `--llm`) and environment (`GGML_METAL_TENSOR_DISABLE=1` on an M5 with Metal) and `POST /sdcpp/v1/img_gen` body; output parsing (records, step lines, tile passes, diagnostic tail, exit classification, GPU-fault markers); progress tracking; request validation (Qwen-Image on Metal ≤ 1 MP); per-family workflows and `workflowsForSpec`; engine gating (Qwen Image 2.1 and Krea 2 Turbo need build ≥ 883, `ENGINE_UPDATE_REQUIRED`) and install selection | `src/diffusion/{args,progress,tracker,validate,workflow,compat}.ts` | plugin `{args,progress,jobs,session}.rs` | the Rust `#[test]` tables ported by hand into the sibling tests; `test/fixtures/sdcpp/required-flags.txt` |
| Gallery files: `<outputDir>/<jobId>-<index:02>.png` with `tEXt` chunks `atomic` (the `ImageRecipe` JSON, every key, `null`s) and `parameters` (A1111 line) right after `IHDR`; `<id>.thumb.png` (≤256 px); `.flags.json` `{id:{pinned,archived}}`. A PNG without a valid `atomic` recipe is foreign: never listed, never deleted. A batch with an all-black or all-white frame is refused before anything is saved (`INVALID_OUTPUT`); such an item an older build saved is hidden when its thumbnail (or PNG) is ≤ 128 KiB | `src/diffusion/{gallery,recipe,png}.ts` | plugin `gallery.rs` at `767ff6350` | ported `gallery.rs` tables; `test/fixtures/png/` (independent encoder, Pillow-checked); read-only run against images written by app v2.0.40 |
...
| `sd-server` supervision: ready when `GET /v1/models` answers 200 (polled every 300 ms within `startupTimeoutSecs`), `GET /sdcpp/v1/capabilities` read afterwards (404 = another process), SIGTERM → 5 s → SIGKILL; jobs `POST /sdcpp/v1/img_gen` (202 `{id}`, 429 = queue full, 400 = rejected), `GET /sdcpp/v1/jobs/<id>` every 400 ms, `POST …/cancel`, 6 h ceiling; the cancel ladder (native → grace → server stopped, spec kept); a GPU fault in the job's output stops the server with the spec kept (`gpu-fault`); an engine install finalized under the load lock unloads a model on a replaced tree (`engine-updated`); a load refused for its engine is `failed` with reason `load-blocked` | `src/diffusion/{server-process,jobs,session,service}.ts` | plugin `{process,jobs,session,commands,lib}.rs` at `767ff6350`; upstream `examples/server/routes_sdcpp.cpp` | ported `jobs.rs` and `process.rs` tables; `test/helpers/fake-sd-server.mjs` end to end |
...
| ZCode's provider file for `launch zcode`: `<dataBase>/.zcode/v2/provider_config.json` (`schemaVersion: 1`), only `providerId: atomic-chat` entries replaced, keys sorted, owner-only, written under ZCode's `<file>.lock/owner-<token>.json` lock, a one-time `.atomic-backup` | `src/integrations/configure/zcode.ts`, `src/integrations/catalog.ts` | app `src-tauri/src/core/system/commands.rs` (`configure_zcode`) and `cli/integrations.rs` at `ec1fd3ea7`; ZCode's `provider-config-file-codec.ts` | ported `zcode_tests` (`zcode.test.ts`); no golden fixtures, the emitter left with the app's Rust CLI |
