...
const run = (args: string[]) => core.runCli(dataFolder, args)
const runAsync = (args: string[]) => core.runCliAsync(dataFolder, args)
const startDaemon = (extra: string[] = []) => core.startDaemon(dataFolder, daemons, extra)
const control = (ready: ReadyLine, path: string, init: RequestInit = {}) =>
  core.control(dataFolder, ready, path, init)
const writeModel = (id: string) => core.writeModel(dataFolder, id)
const writeFakeBackend = () => core.writeFakeBackend(dataFolder)
