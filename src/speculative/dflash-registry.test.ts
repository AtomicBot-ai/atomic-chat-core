...
import { checkDflashSupport, listDflashDrafts, resolveDflashDraft } from './dflash-registry.js'

describe('DFlash registry', () => {
  it('defaults to Atomic Chat Q8_0 across target quantizations', () => {
    const draft = resolveDflashDraft('unsloth/Qwen3_5-9B-GGUF-Qwen3_5-9B-IQ4_XS')
  ...
  it('uses Atomic Chat Q8_0 drafts when published', () => {
    const expectedRepos = new Map([
      ...
      ['Qwen3-Coder-30B-A3B-Instruct', 'AtomicChat/Qwen3-Coder-30B-A3B-DFlash-GGUF'],
      ['Qwen3-Coder-Next-Instruct', 'AtomicChat/Qwen3-Coder-Next-DFlash-GGUF'],
    ])
  ...
  it('falls back to the only published quant for Q8_0-only families', () => {
    ...
    expect(resolveDflashDraft('Qwen3-Coder-Next-Instruct')?.quant).toBe('Q8_0')
  })
  ...
  it('uses mainline llama.cpp DFlash GGUF repositories', () => {
    ...
    expect(resolveDflashDraft('Qwen3.6-35B-A3B-Q4_K_M', 'Q4_K_M')?.repo).toBe(
