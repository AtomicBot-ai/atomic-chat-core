...
import { normalizeBaseId, resolveDflashDraft } from './mlx-dflash-registry.js'
import { resolveMtpDraft } from './mlx-mtp-registry.js'
import { resolveEagle3Draft } from './mlx-eagle3-registry.js'
...
describe('normalizeBaseId', () => {
  it('strips standard quant suffixes', () => {
    ...
    expect(normalizeBaseId('mlx-community/Qwen3.5-9B-MLX-bf16')).toBe('qwen3.5-9b')
    expect(normalizeBaseId('mlx-community/gemma-4-E4B-it-4bit')).toBe('gemma-4-e4b')
  })
  ...
  it('strips -qat suffix (ATO-235: QAT models not found in registry)', () => {
    ...
    expect(normalizeBaseId('mlx-community/gemma-4-E4B-it-qat-4bit')).toBe('gemma-4-e4b')
    expect(normalizeBaseId('mlx-community/gemma-4-E2B-it-qat-4bit')).toBe('gemma-4-e2b')
    expect(normalizeBaseId('mlx-community/gemma-4-26B-A4B-it-qat-4bit')).toBe('gemma-4-26b-a4b')
    expect(normalizeBaseId('mlx-community/gemma-4-31B-it-qat-4bit')).toBe('gemma-4-31b')
  })

  it('strips bare -qat with no trailing quant type', () => {
    expect(normalizeBaseId('mlx-community/gemma-4-E4B-it-qat')).toBe('gemma-4-e4b')
  })

  it('strips -unquantized suffix', () => {
    expect(normalizeBaseId('mlx-community/gemma-4-E4B-it-unquantized')).toBe('gemma-4-e4b')
  })

  it('handles qat with various quant bit-depths', () => {
    expect(normalizeBaseId('mlx-community/gemma-4-E4B-it-qat-8bit')).toBe('gemma-4-e4b')
    expect(normalizeBaseId('mlx-community/gemma-4-E4B-it-qat-6bit')).toBe('gemma-4-e4b')
    expect(normalizeBaseId('mlx-community/Qwen3.6-27B-qat-4bit')).toBe('qwen3.6-27b')
  })
...
describe('resolveMtpDraft with QAT targets (ATO-235)', () => {
  ...
  it('resolves gemma-4-26B-A4B QAT target to its MTP assistant', () => {
    const result = resolveMtpDraft('mlx-community/gemma-4-26B-A4B-it-qat-4bit')
    expect(result).not.toBeNull()
    expect(result?.repo).toBe('mlx-community/gemma-4-26B-A4B-it-assistant-bf16')
  })
...
describe('resolveEagle3Draft with QAT targets (ATO-235)', () => {
  ...
  it('resolves gemma-4-26B-A4B QAT target to its EAGLE-3 speculator', () => {
    const result = resolveEagle3Draft('mlx-community/gemma-4-26B-A4B-it-qat-4bit')
    expect(result).not.toBeNull()
    expect(result?.repo).toBe('RedHatAI/gemma-4-26B-A4B-it-speculator.eagle3')
  })
})
...
describe('resolveDflashDraft with QAT targets (ATO-235)', () => {
  ...
  it('resolves gemma-4-26B-A4B QAT target to its DFlash draft', () => {
    const result = resolveDflashDraft('mlx-community/gemma-4-26B-A4B-it-qat-4bit')
    expect(result).not.toBeNull()
