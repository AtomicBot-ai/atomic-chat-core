...
describe('planMlxShardRepair', () => {
  it('claims the legacy file for the single absent shard', () => {
    expect(planMlxShardRepair(weightMap, [...otherFiles, 'model.safetensors', SHARD_2])).toEqual({
  ...
  it('leaves an intact checkpoint alone', () => {
    expect(planMlxShardRepair(weightMap, [...otherFiles, SHARD_1, SHARD_2])).toBeUndefined()
  })

  it('leaves an interrupted download alone', () => {
    expect(planMlxShardRepair(weightMap, [...otherFiles, 'model.safetensors'])).toBeUndefined()
  })

  it('does nothing without the legacy file', () => {
    expect(planMlxShardRepair(weightMap, [...otherFiles, SHARD_2])).toBeUndefined()
  })

  it('does not touch a checkpoint whose index names the legacy file', () => {
    expect(
      planMlxShardRepair({ 'a.weight': 'model.safetensors', 'b.weight': SHARD_2 }, [
        ...otherFiles,
        'model.safetensors',
      ])
    ).toBeUndefined()
  ...
  it('tolerates a malformed index', () => {
    ...
    expect(planMlxShardRepair('nonsense', ['model.safetensors'])).toBeUndefined()
    expect(planMlxShardRepair({ 'a.weight': 42 }, ['model.safetensors'])).toBeUndefined()
  })
})

describe('repointLegacyWeightPath', () => {
  it('rewrites a path that still names the legacy file', () => {
    expect(repointLegacyWeightPath('mlx/models/gemma/model.safetensors', SHARD_1)).toBe(
      `mlx/models/gemma/${SHARD_1}`
    )
  })

  it('rewrites Windows-separated paths', () => {
    expect(repointLegacyWeightPath('C:\\data\\mlx\\gemma\\model.safetensors', SHARD_1)).toBe(
      `C:\\data\\mlx\\gemma\\${SHARD_1}`
    )
  })

  it('leaves other paths untouched', () => {
    expect(repointLegacyWeightPath('mlx/models/gemma', SHARD_1)).toBe('mlx/models/gemma')
    expect(repointLegacyWeightPath(`mlx/models/gemma/${SHARD_2}`, SHARD_1)).toBe(
      `mlx/models/gemma/${SHARD_2}`
    )
    expect(repointLegacyWeightPath('model.safetensors', SHARD_1)).toBe('model.safetensors')
  })
})
