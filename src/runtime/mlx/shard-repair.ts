/**
 * Healing MLX checkpoints downloaded before the shard-naming fix.
 *
 * Ported from: extensions/mlx-extension/src/shardRepair.ts (verbatim logic).
 */
...
export function planMlxShardRepair(
  ...
  return { from: LEGACY_WEIGHT_FILE, to: absent[0] as string }
}
...
export function repointLegacyWeightPath(path: string, shardFileName: string): string {
  const separator = path.slice(-LEGACY_WEIGHT_FILE.length - 1, -LEGACY_WEIGHT_FILE.length)
  if (!path.endsWith(LEGACY_WEIGHT_FILE) || (separator !== '/' && separator !== '\\')) {
    return path
  }
