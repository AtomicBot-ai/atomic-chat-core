/**
 * The settings of one MLX load, turned into the six values mlx-server is started with.
 *
 * Ported from: extensions/mlx-extension/src/buildMlxConfig.ts (verbatim logic).
 */

import { DEFAULT_CTX_LEN } from '../llamacpp/ctx-ladder.js'

/** What `build_mlx_server_args` reads (tauri-plugin-mlx/src/commands.rs `MlxConfig`). */
...
export function selectMlxDraftSettings(config: MlxExtensionConfigInput): MlxDraftSelection {
 ...
 */
export function resolveMlxCtxSize(ctxSize: Numeric | undefined, maxCtxTrain: number | undefined): number {
  const trainMax = typeof maxCtxTrain === 'number' && maxCtxTrain > 0 ? maxCtxTrain : undefined
  const explicit = asNumber(ctxSize)
  if (explicit !== undefined && explicit > 0) {
    return trainMax ? Math.min(explicit, trainMax) : explicit
  }
  return trainMax ? Math.min(trainMax, MLX_DEFAULT_CTX_CAP) : MLX_DEFAULT_CTX_FALLBACK
}
...
export function buildMlxConfig(
  ...
  const kvScheme =
    config.kv_quant_scheme === 'uniform' || config.kv_quant_scheme === 'turboquant'
      ? config.kv_quant_scheme
      : ''
  const kvBitsRaw = asNumber(config.kv_bits)
  const kvBits = kvScheme && kvBitsRaw !== undefined && kvBitsRaw > 0 ? kvBitsRaw : 0
