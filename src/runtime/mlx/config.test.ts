...
describe('selectMlxDraftSettings', () => {
  it.each([
    ['dflash', { dflash_enabled: true, draft_model_path: 'draft/dflash', block_size: 16 }, 16],
    ['mtp', { mtp_enabled: true, draft_model_path: 'draft/mtp', mtp_block_size: 4 }, 4],
    [
      'eagle3',
      {
        eagle3_enabled: true,
...
describe('buildMlxConfig', () => {
  it('removes all draft arguments when the resolved path is empty', () => {
    expect(buildMlxConfig({ ctx_size: 32_768 }, { draftKind: 'mtp', draftPath: '  ', blockSize: 4 })).toEqual(
      {
        ctx_size: 32_768,
        draft_model_path: '',
        block_size: 0,
  ...
  it.each([
    ...
    ['uniform', 0, '', 0],
    ['turboquant', -1, '', 0],
  ])('normalizes KV scheme %s with bits %s', (scheme, bits, expectedScheme, expectedBits) => {
...
describe('resolveMlxCtxSize', () => {
  ...
  it('is what buildMlxConfig loads with', () => {
    expect(
      buildMlxConfig({}, { draftKind: 'dflash', draftPath: '', blockSize: 0 }, { maxCtxTrain: 40_960 })
        .ctx_size
