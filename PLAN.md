...
| Выход | `make verify` с supervisor-тестами (fake core); смерть ядра → инвалидирование snapshot → один рестарт; force-quit приложения оставляет CLI-модель работающей; следующий запуск приложения делает attach. Пропуск >1000 событий и перезапуск seq требуют resync. `ATOMIC_CORE_CMD="bun run ../atomic-chat-core/src/cli/bin.ts" yarn dev` работает |
